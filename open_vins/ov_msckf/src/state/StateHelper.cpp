...
void StateHelper::EKFPropagation(std::shared_ptr<State> state, const std::vector<std::shared_ptr<Type>> &order_NEW,
    ...
    // std::cout << "After Propagation Cov row 0,0 = \n " << state->_Cov(0,0) << std::endl;
    // std::cin.get();


}

void StateHelper::CKF_update(std::shared_ptr<State> state,
                                const Eigen::MatrixXd sigma_points_, 
                                const Eigen::MatrixXd z_stacked,
                                const Eigen::VectorXd z_measured_big,
                                const Eigen::MatrixXd P,
                                const Eigen::MatrixXd R){

    int num_meas;
    int n;
    num_meas = z_measured_big.rows();
    n = P.rows();
    Eigen::MatrixXd z_hat = z_stacked.colwise().sum() / (2*n); // zhat = [ . . .]
    // std::cout << "z_hat = " << z_hat << std::endl;
    // std::cout << "z_measured_big = " << z_measured_big.transpose() << std::endl;


    // Calculate Pzz
    Eigen::MatrixXd P_zz;
    P_zz = Eigen::MatrixXd::Zero(num_meas, num_meas);
    ...
    for(int i=0; i < 2*n; i++)
    {
        ...
        Eigen::Matrix<double, 4, 1> del_q = ov_core::quat_multiply(IMU_sigma_q, IMU_q_inv);
        ...
        for (const auto& [key,value]:state->_clones_IMU){ //for each IMU clone
            ...
            Eigen::Matrix<double, 4, 1> del_q = ov_core::quat_multiply(IMU_clone_sigma_q, IMU_clone_q_inv);
    ...
    Eigen::MatrixXd res;
    res = z_measured_big - z_h_trans; // column vector

    // Text file
    // std::ofstream myfile ("/home/zahid/openvins_txt.txt", std::ios::app);
    // if (myfile.is_open())
    // {
    //   myfile << " imu_state_ \n";
    //   myfile << state->_imu->quat()[0]<< "," << state->_imu->quat()[1] << "," << state->_imu->quat()[2] << "," << state->_imu->quat()[3] << "\n";
    //   myfile << state->_imu->pos()[0]<< "," << state->_imu->pos()[1] << "," << state->_imu->pos()[2] << "\n";
    //   myfile << state->_imu->vel()[0]<< "," << state->_imu->vel()[1] << "," << state->_imu->vel()[2] << "\n";
    //   myfile << state->_imu->bias_g()[0]<< "," << state->_imu->bias_g()[1] << "," << state->_imu->bias_g()[2] << "\n";
    //   myfile << state->_imu->bias_a()[0]<< "," << state->_imu->bias_a()[1] << "," << state->_imu->bias_a()[2] << "\n";

    
    //   myfile << " cam_states_ \n";
    //   for (const auto& [key,value]:state->_clones_IMU){
    //       std::shared_ptr<PoseJPL> clone_Ii = value;
    //       myfile << clone_Ii->quat()[0]<< "," << clone_Ii->quat()[1] << "," << clone_Ii->quat()[2] << "," << clone_Ii->quat()[3] << "\n";
    //       myfile << clone_Ii->pos()[0]<< "," << clone_Ii->pos()[1] << "," << clone_Ii->pos()[2] << "\n";
    //   }
    ...
    // std::cout << " quat  = " << state->_imu->quat() << std::endl;
    // std::cout << " bg  = " << state->_imu->bias_g() << std::endl;
    // std::cout << " vel  = " << state->_imu->vel() << std::endl;
    // std::cout << " ba  = " << state->_imu->bias_a() << std::endl;
    // std::cout << " pos  = " << state->_imu->pos() << std::endl;
    // std::cout << " P det  = " << P.determinant() << std::endl;
    // std::cin.get();

    // Calculate Kalman Gain
    Eigen::MatrixXd P_zz_inv = Eigen::MatrixXd::Identity(P_zz.rows(), P_zz.rows());
    P_zz.selfadjointView<Eigen::Upper>().llt().solveInPlace(P_zz_inv);
    Eigen::MatrixXd K = P_xz * P_zz_inv.selfadjointView<Eigen::Upper>(); // Kalman gain
    // Eigen::MatrixXd K = P_xz * P_zz.inverse();

    Eigen::VectorXd diags_zz = P_zz.diagonal();
    for(int i=0; i<diags_zz.rows(); i++) {
        if(diags_zz(i) < 0.0) {
            printf(RED "P_zz - diagonal at %d is %.2f\n" RESET,i,diags_zz(i));
        }
    }

    // Calculate our delta and update all our active states
    Eigen::VectorXd deltaX = K * res;

    for (size_t i = 0; i < state->_variables.size(); i++) { // loop over all states. i.e. imu, imu_clones
        state->_variables.at(i)->update(deltaX.block(state->_variables.at(i)->id(), 0, state->_variables.at(i)->size(), 1));
    }

    // std::cout << " Pzz = \n " << P_zz << std::endl;
    // std::cout << " .................................. \n "<< std::endl;
    // std::cout << " K = \n " << K.row(2).transpose() << std::endl;
    // // std::cout << " K = \n " << K.col(2).transpose() << std::endl;
    // std::cout << " .................................. \n "<< std::endl;
    // std::cout << " K = \n " << K.col(K.cols()-1).transpose() << std::endl;
    // std::cout << " .................................. \n "<< std::endl;
    // std::cout << " res = \n " << res.transpose() << std::endl;
    // std::cout << " .................................. \n "<< std::endl;
    // std::cout << " mul = \n " << (K.row(2)*res).transpose() << std::endl;
    ...
    // std::cout << " K = \n" << K.row(0) << std::endl;
    // std::cout << " K col = \n" << K.block(0,0,10,10) << std::endl;
    // std::cout << " K col = \n" << K.block(0,5,5,5) << std::endl;
    // std::cin.get();

    // for (int i=0; i<K.rows(); i++){
    //     for( int j=0; j < K.cols(); j++){
    //         if (abs(K(i,j)) <= 0.0000001){
    //             std::cout << "K at " << i << "," << j << " is zero \n"<< std::endl;
    ...
    Eigen::MatrixXd K_trnas = K.transpose();
    Eigen::MatrixXd P_t_K_trnas = P_zz* K_trnas;
    Eigen::MatrixXd Cov_updated = K * P_t_K_trnas;
    // Enforce symmetry
    Eigen::MatrixXd Cov_up_corrected_transpose = Cov_updated.transpose();
    Eigen::MatrixXd Cov_up_corrected;
    Cov_up_corrected = Cov_updated + Cov_up_corrected_transpose;
    Cov_up_corrected /= 2;
    Cov_updated = Cov_up_corrected;

    state->_Cov  -= Cov_updated;
    // state->_Cov.triangularView<Eigen::Upper>() -= K * P_zz * K.transpose();
    // state->_Cov = state->_Cov.selfadjointView<Eigen::Upper>();
    // std::cout << " Cov row 3 = \n " << state->_Cov.row(3) << std::endl;

    // std::cout << " K * P_zz * K.transpose() = \n " << Cov_updated(0,0) << std::endl;

    // std::cout << " K * P_zz * K.transpose() 10x10 = \n " << Cov_updated.block(0,0,10,10) << std::endl;
    // state->_Cov -= K * P_zz * K.transpose();
    ...
    for(int i=0; i<diags.rows(); i++) {
        if(diags(i) < 0.0) {
            printf(RED "StateHelper::UKFUpdate() - diagonal at %d is %.2f\n" RESET,i,diags(i));
            found_neg = true;
        }
    }

    // std::cout << " Bias of covarience = \n" << diags.block(0,0,6,1) << std::endl;
    // // std::cout << " deltaX = \n" << deltaX.block(0,0,6,1) << std::endl;
    // std::cin.get();
    assert(!found_neg);
...
void StateHelper::EKFUpdate(std::shared_ptr<State> state, const std::vector<std::shared_ptr<Type>> &H_order,
    ...
    S.triangularView<Eigen::Upper>() = H * P_small * H.transpose();  /// MSCKF line 2077
    ...
    // std::cout << " Cov row 7,7 = \n " << state->_Cov(7,7) << std::endl;
    // std::cin.get();
    // std::cout << "K.rows() = \n" << K.rows() << std::endl;
    // std::cout << "state->_Cov.rows() =\n" << state->_Cov.rows() << std::endl;
    ...
    // std::cout << "state->_variables.size()" << state->_variables.size() << std::endl;
    ...
    for (size_t i = 0; i < state->_variables.size(); i++) { // loop over all states. i.e. imu, imu_clones
        // std::cout << "taking from dx block start = \n" << state->_variables.at(i)->id() << " X 0" << std::endl;
        // std::cout << "size = \n" << state->_variables.at(i)->size() << " X 1" << std::endl;
        // std::cout << "dx to update = \n" << dx.block(state->_variables.at(i)->id(), 0, state->_variables.at(i)->size(), 1) << std::endl;
        state->_variables.at(i)->update(dx.block(state->_variables.at(i)->id(), 0, state->_variables.at(i)->size(), 1));
        // std::cin.get();
    }
    // std::cout << " Bias of covarience = \n" << diags.block(0,0,6,1) << std::endl;
    // std::cout << " dx = \n" << dx.block(0,0,6,1) << std::endl;
    // std::cin.get();
...
Eigen::MatrixXd StateHelper::get_marginal_covariance(std::shared_ptr<State> state, const std::vector<std::shared_ptr<Type>> &small_variables) {
    ...
    // std::cout << " small_variables.size() \n" <<  small_variables.size() << std::endl;
    // std::cin.get();
    ...
    // std::cout << "size of covariance" << Small_cov.rows() << "x" <<Small_cov.rows() << std::endl;
    // std::cin.get();
