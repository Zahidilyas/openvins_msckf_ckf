...
namespace ov_core {
    ...
    inline Eigen::Matrix<double, 4, 1> quat_multiply_my(const Eigen::Matrix<double, 4, 1> &q, const Eigen::Matrix<double, 4, 1> &p) {
        Eigen::Matrix<double, 4, 1> q_t;

        Eigen::Quaternion<double> quat;
        Eigen::Quaternion<double> quat_2;
        Eigen::Quaternion<double> result;
        quat.vec() = q.block(0, 0, 3, 1);
        quat.w() = q(3, 0);

        quat_2.vec() = p.block(0, 0, 3, 1);
        quat_2.w() = p(3, 0);

        result = quat*quat_2;
        result.normalize();

        q_t.block(0, 0, 3, 1) = result.vec();
        q_t(3,0) = result.w();
